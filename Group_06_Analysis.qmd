...
The @fig-boxplot-ratings distribution of rating genre
...
$$\begin{aligned}\ln\left(\frac{p_i}{1-p_i}\right) &= \alpha + \beta_1 \cdot \text{year}_i + \beta_2 \cdot \text{length}_i + \beta_3 \cdot \text{budget}_i \\&\quad + \beta_{\text{Animation}} \cdot \mathbb{I}_{\text{Animation}}(i) + \beta_{\text{Comedy}} \cdot \mathbb{I}_{\text{Comedy}}(i) \\&\quad + \beta_{\text{Document}} \cdot \mathbb{I}_{\text{Document}}(i) + \beta_{\text{Drama}} \cdot \mathbb{I}_{\text{Drama}}(i) \\&\quad + \beta_{\text{Romance}} \cdot \mathbb{I}_{\text{Romance}}(i) + \beta_{\text{Short}} \cdot \mathbb{I}_{\text{Short}}(i)\end{aligned}$$
...
For asuumption 2, since the observation belong to
independent fils, it is satisfied. For assumption 3,
...
$$\text{Odds}(p_i) = \frac{p_i}{1 - p_i} = \exp\left(\alpha + \beta_1 \cdot \text{length}_i + \beta_2 \cdot \text{budget}_i + \right. \\
\quad + \beta_{\text{Animation}} \cdot \mathbb{I}_{\text{Animation}}(i) + \beta_{\text{Comedy}} \cdot \mathbb{I}_{\text{Comedy}}(i) 
\quad + \left. \beta_{\text{Document}} \cdot \mathbb{I}_{\text{Document}}(i) + \beta_{\text{Drama}} \cdot \mathbb{I}_{\text{Drama}}(i) + \right. \\
...
write.csv(imdb_data, file = "Group_06_Data.csv") 
```

Let's have a look at the first five rows of the new
data frame.

```{r}
#| label: tbl-dataset-final
#| tbl-cap: Glimpse of the first five rows in the IMDB data set with log-odds, odds and probabilities
imdb_data |>
  slice_head(n=5) |>
  gt()
```
...
In comparison to the other models, m2_model has the
minimum BIC. Since BIC places a stronger penalty on
model complexity than AIC for smaller data sets,
simpler model is chosen. m2_model also performed
slightly better in residual tests. All the parameters
of m2_model are also statistically significant.It is
...
In summary, it can be observed that 'length', 'budget'
and 'genre' are the properties of film that influence
the IMDB ratings to be greater than 7 on not.
