...
$$ 
\ln\left(\frac{p_i}{1-p_i}\right)=\alpha + \beta_1 \cdot \textrm{year}_i+\beta_2 \cdot \textrm{length}_i+\beta_3 \cdot \textrm{budget}_i+\beta_4 \cdot \textrm{log_votes}_i
...
$$\mathbb{I}_{\mbox{genre}}(i)=\left\{
                \begin{array}{ll}
                  1 ~~~ \mbox{if genre of} ~ i \mbox{th observation is Genre},\\
                  0 ~~~ \mbox{Otherwise}.\\
                \end{array}
              \right.\\
...
1.  [Deviance]{.underline} : for a GLM model that fits the data well the approximate deviance D is $\chi^2(m-p)$ where *m* is the number of parameters in the saturated model (full model) and *p* is the number of parameters in the model of interest. In the above model, 2463.5-1018.0 is larger than 95th percentile of the $\chi^2(1936-1926)$ . There is no evidence of lack of fit.
    ...
    However, for a binary response neither D nor $\chi^2$ provide a useful measure measure of goodness of fit
...
for (i in 5:15) {
  print(HLTest(m0_model, g = i)$p.value)
  }
```

A small p-value suggests that the fitted model is not an adequate model. Conversely, large p-value indicated no lack of it. From the above output there is evidence of lack of fit.
...
The first two assumptions are self explanatory and depend on the study design. For assumption 3, the scatterplot justified.
...
#| message: false
# Cook's Distance
c0 <- plot(m0_model, which=4, id.n = 10)

```

Top model has outliers which can be observed in @fig-cook-m0 .This is due to the outliers present in the data set.
...
for (i in 5:25) {
  print(HLTest(m1_model, g = i)$p.value)
  }
```

From the above output there is evidence of lack of fit.
...
$$\mathbb{I}_{\mbox{genre}}(x)=\left\{
                \begin{array}{ll}
                  1 ~~~ \mbox{if } ~ x \mbox{th observation is genre},\\
                  0 ~~~ \mbox{Otherwise}\\
                \end{array}
              \right.\\
              genre=\{Animation,Comedy,Documentary,Drama,Romance,Short\}$$
...
# Hosmer-Lemeshow goodness of fit test
for (i in 5:15) {
  print(HLTest(m2_model, g = i)$p.value)
  }
```
...
```{r}
# 1. Likelihood Ratio Chi-Squared Statistic Test
lr_test_1 <- anova(m0_model, m1_model, test = "Chisq") # implies log_votes is not significantly associated with the outcome (p>0.05) therefore m1_model better than m0_model
lr_test_1
lr_test_2 <- anova(m1_model, m2_model, test = "Chisq") # p <0.05 
lr_test_2
```
